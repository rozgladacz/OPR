...
def _available_unit_abilities(db: Session) -> list[str]:
    abilities = set(costs.get_known_unit_abilities())
    existing = db.execute(select(models.Unit.flags)).scalars().all()
    for entry in existing:
        for ability in costs.parse_unit_abilities(entry):
            if ability:
...
def _army_context(
    ...
    available_abilities = _available_unit_abilities(db)
    return {
        "request": None,
        "user": current_user,
        "army": army,
...
def _clean_abilities(raw: str | None, available: list[str]) -> tuple[str | None, list[str]]:
    abilities = costs.parse_unit_abilities(raw)
    allowed = {_normalize_token(option) for option in available}
    invalid = [ability for ability in abilities if _normalize_token(ability) not in allowed]
    cleaned = ",".join(abilities) if abilities else None
...
def view_army(
    ...
    context = _army_context(db, army, current_user)
    context["request"] = request
    return templates.TemplateResponse("army_edit.html", context)
...
def add_unit(
    ...
    available_abilities = _available_unit_abilities(db)
    cleaned_flags, invalid = _clean_abilities(flags, available_abilities)
    if invalid:
        context = _army_context(
            db,
            army,
            current_user,
            error="Nieznane zdolności: " + ", ".join(invalid),
            new_unit={
                "name": name,
                "quality": quality,
                "defense": defense,
        ...
        context["request"] = request
        return templates.TemplateResponse("army_edit.html", context, status_code=400)

...
def init_db() -> None:
    ...
    with SessionLocal() as session:
        ...
        if not session.execute(select(models.Weapon)).first():
            rifle = models.Weapon(
                name="Karabin",
                range=costs.normalize_range_value("24"),
                attacks=2,
                ap=1,
                tags="Ranged",
            ...
            sword = models.Weapon(
                ...
                range=costs.normalize_range_value("0"),
                attacks=1,
                ap=2,
                tags="Melee",
                notes="Energia tnąca",
            ...
            rifle.cached_cost = costs.weapon_cost(rifle, quality=4)
            sword.cached_cost = costs.weapon_cost(sword, quality=4)
